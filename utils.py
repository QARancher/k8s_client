...
def underscore_to_uppercase(dict_to_edit):
    ...
    if isinstance(dict_to_edit, list):
        return [value for value in (underscore_to_uppercase(value)
                                    for value in dict_to_edit) if value]
    ...
    return {key: value for key, value in
            ((key, underscore_to_uppercase(value))
             for key, value in dict_to_edit.items()) if value}
...
def field_filter(obj_list,
                 field_selector):
    return [obj for obj in obj_list if selector(obj=obj,
                                                field_selector=field_selector)]


def selector(obj,
             field_selector):
    return all(condition(obj=obj,
                         field_selector=field.strip())
               for field in field_selector.split(","))
